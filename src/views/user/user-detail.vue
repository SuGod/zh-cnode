<template>
  <mu-paper :z-depth="1" style="height: 100%">
    <mu-appbar color="primary" title="用户信息">
    ...
    <mu-list textline="two-line" v-show="active === 0" style="height: calc(100% - 100px)">
      <template v-for="recentTopic of user.recent_topics">
        <mu-list-item avatar button :key="`${recentTopic.id}recentTopic`">
          <mu-list-item-action>
    ...
    <mu-list textline="two-line" v-show="active === 1">
      <template v-for="replyTopic of user.recent_replies">
        <mu-list-item avatar button :key="`${replyTopic.id}replyTopic`">
          <mu-list-item-action>
